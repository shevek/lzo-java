...
public class BlockCompressorStream extends CompressorStream {
    ...
    private static final Log LOG = LogFactory.getLog(BlockCompressorStream.class);
  ...
  public void write(byte[] b, int off, int len) throws IOException {
    ...
    if (len + limlen > MAX_INPUT_SIZE && limlen > 0) {
        LOG.info("Flushing existing data.");
    ...
    if (len > MAX_INPUT_SIZE) {
      ...
      // not exceeding the maximum size until it is exhausted.
        LOG.info("Big data writes length " + len);
      rawWriteInt(len);
      do {
  ...
  public void finish() throws IOException {
    if (!compressor.finished()) {
        LOG.info("Finish writes length " + compressor.getBytesRead());
      rawWriteInt((int)compressor.getBytesRead());
      compressor.finish();
      while (!compressor.finished()) {
        compress();
  ...
  protected void compress() throws IOException {
    ...
    if (len > 0) {
      // Write out the compressed chunk
        LOG.info("Compress writes length " + len);
      rawWriteInt(len);
      out.write(buffer, 0, len);
    }
 else LOG.info("Wrote no compressed block.");
  }
  
  private void rawWriteInt(int v) throws IOException {
      // LOG.info("Writing " + v, new Exception());
    out.write((v >>> 24) & 0xFF);
    out.write((v >>> 16) & 0xFF);
    out.write((v >>>  8) & 0xFF);
    out.write((v >>>  0) & 0xFF);
